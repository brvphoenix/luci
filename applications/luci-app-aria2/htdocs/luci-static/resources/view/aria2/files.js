...
return view.extend({
	load: function() {
		return uci.load('aria2').then(function() {
			...
			return Promise.all([
				fs.exec_direct('/bin/cat', [ config_file ]).then(function(res) { return { 'file': config_file, 'rows': res.trim().split(/\n/).length, 'content': res.trim() } }),
				fs.exec_direct('/bin/cat', [ session_file ]).then(function(res) { return { 'file': session_file, 'rows': res.trim().split(/\n/).length, 'content': res.trim() } })
			]);
		})
	},
		...
		return E('div', {'class': 'cbi-map'}, [
			...
			E('div', {'class': 'cbi-section'}, [
				E('div', {'class': 'cbi-section-descr'},
					E('p', {}, _('Content of config file: <code>%s</code>.').format(data[0].file))
				),
				E('textarea', {
					'id': 'custom_config',
					'style': 'width: 100%',
			...
			E('div', {'class': 'cbi-section'}, [
				E('div', {'class': 'cbi-section-descr'},
					E('p', {}, _('Content of session file: <code>%s</code>.').format(data[1].file))
				),
				E('textarea', {
					'id': 'custom_config',
					'style': 'width: 100%',
