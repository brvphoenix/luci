...
CBIAria2Status = form.DummyValue.extend({
	renderWidget: function() {
		...
		poll.add(function() {
			Promise.all([
				callServiceList('aria2', extra).then(function(res) {
					var stat = res.running ? _('The Aria2 service is running.') : _('The Aria2 service is not running.');
					return E('p', {}, E('em', {}, stat));
				}),
				getWebFrontInstalled().then(function(installed) {
					var btn = E('p', _('Installed web interface: '));
					for (var i in installed) {
						btn.appendChild(E('button', {
							'class': 'cbi-button btn',
							'click': openWebInterface.bind(this, i)
						}, installed[i]));
					}
					return Object.keys(installed).length > 0 ? btn : null;
				})
			]).then(function(res) {
		...
		dom.append(node, [
			...
			E('span', { 'class': 'control-group' },
				E('button', {
					...
					'click': this.clickFn.bind(this, 'widget.' + this.cbid(section_id))
				}, this.btnTitle)
			)
		]);
		return node;
...
CBIRpcUrl = form.DummyValue.extend({
	renderWidget: function(section_id, widget_id, cfgvalue) {
		var widget_id = 'widget.' + this.cbid(section_id);
		return E([], [
			E('input', {
				'id': widget_id,
				'type': 'text',
			...
			E('span', { 'class': 'control-group' }, [
				E('button', {
					'class': 'cbi-button cbi-button-neutral',
					'click': this.clickFn.bind(this, widget_id, 0)
				}, _('Show HTTP(s) URL')),
				E('button', {
					'class': 'cbi-button cbi-button-neutral',
					'click': this.clickFn.bind(this, widget_id, 1)
				}, _('Show WebSocket(s) URL'))
			])
		]);
	},
});

function getToken(widget_id) {
	...
	document.getElementById(widget_id).value = randomStr;
};
...
function getWebFrontInstalled() {
	...
	for (var s in supported) {
		actions.push(fs.exec_direct('/bin/grep', [ 'Package: ' + s, '/usr/lib/opkg/status' ])
		.then(L.bind(function(s, res) {
			return res ? [s, supported[s]] : [];
		}, this, s)));
	}
...
function showRPCURL(widget_id, useWS) {
	var getOptVal = L.bind(function(opt, default_val) {
		var res = this.map.lookupOption.call(this.map, opt, this.section.section, this.config);
		default_val = default_val || null;
		return (res && res[0].formvalue(res[1])) ? res[0].formvalue(res[1]) : default_val;
	}, this);
	...
	document.getElementById(widget_id).value = url;
};
