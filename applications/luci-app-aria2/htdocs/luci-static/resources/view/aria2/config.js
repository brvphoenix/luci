...
CBIRpcUrl = form.DummyValue.extend({
	renderWidget: function(section_id, option_index, cfgvalue) {
		return E([
			E('input', {
				'id': 'widget.' + this.cbid(section_id),
				'type': 'text',
				'readonly': true
			}),
			E('br'),
			E('span', { 'class': 'control-group' }, [
				E('button', {
					'class': 'cbi-button cbi-button-neutral',
					'click': this.clickFn.bind(this, section_id, 0)
				}, 'HTTP(s)'),
				E('button', {
					'class': 'cbi-button cbi-button-neutral',
					'click': this.clickFn.bind(this, section_id, 1)
				}, 'WebSocket(s)')
			])
		]);
	},
...
function showRPCURL(section_id, useWS) {
	...
	document.getElementById('widget.' + this.cbid(section_id)).value = url;
};
